...
const ImageResource = Joi.object({
  sizes: Joi.string()
    .optional()
...
const WebAppManifest = Joi.object({
...
const ManifestPluginConfig = Joi.object({
  icon: Joi.string(),
...
export async function validateAll(pluginOptions) {
  const merged = WebAppManifest.concat(ManifestPluginConfig)
  return await merged.validateAsync(pluginOptions, { allowUnknown: false })
}

export async function extractConfig(pluginOptions) {
  return ManifestPluginConfig.validateAsync(pluginOptions, {
    allowUnknown: true,
    stripUnknown: true,
  })
}

export async function extractManifest(pluginOptions) {
  return WebAppManifest.validateAsync(pluginOptions, {
    allowUnknown: true,
    stripUnknown: true,
  })
}
